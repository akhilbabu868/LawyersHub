...
$(document).ready(function () {
    ...
    viewusertable = $("#viewUserDataTable").dataTable();
    displayUser();
                        ...
                        "success": function (res, stat)
                        {
                            ...
                            for (var i = 0; i < res.aaData.length; i++)
                            {
                                ...
                                if (res.aaData[i].loginStatus == 1) {
                                    res.aaData[i][4] = "<button type=\"button\" class=\"btn btn-danger\"  onclick=\"deactivate('" + logid1+ "','" + status + "')\">Deactivate</button>";
                                } else {
                                    res.aaData[i][4] = "<button type=\"button\" class=\"btn btn-success\"  onclick=\"activate('" + logid1 + "','" + status + "')\">Activate</button>";
                                }
//                                res.aaData[i][5] = '<div class="btn-group"><input type="button" form="' + i + '"  class="btn btn-info btn-small view " id="' + id1 + '" value="View" >';
                                res.aaData[i][5] = "<button type=\"button\" data-toggle=\"modal\" data-target=\"#exampleModalCenter\"class=\"btn btn-info btn-small view \"  onclick=\"view('" + id1 + "')\">View</button>";
                                '</div>';
                            }
                            fnCallback(res);
                        }
